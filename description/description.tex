...
\item the two hybrid solvers \texttt{exact-printemps} and \texttt{scip-printemps} that pair PRINTEMPS with an off-the-shelf complete PB / MIP solver (Sec.~\ref{sec:hybrid}), together with several safeguards against SCIP's double-precision numerical issues (Sec.~\ref{sec:scip-guards});
