...
\begin{itemize}
\item widened coefficient range from 32-bit to 64-bit signed integers (Sec.~\ref{sec:coeff});
...
In the 2025 submission each OPB coefficient had to fit within the range of a 31-bit signed integer. For 2026 the OPB reader and the internal coefficient/objective accumulators have been migrated to \texttt{int64\_t}, so coefficients up to the range of a 63-bit signed integer are now accepted. This removes a class of overflow-related rejections we encountered on instances whose hard or soft weights are close to $2^{31}$.
